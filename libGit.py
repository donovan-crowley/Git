...
argsp.add_argument("path", nargs = "+", help = "Files ot add")
...
def tag_create(repo, name, ref, create_tag_object = False):
    ...
    if create_tag_object:
        ...
        tag.kblm[None] = b"A tag generated by wyag, which won't let you customize the message!\n"
...
def index_read(repo):
    ...
    for i in range(0, count):
        ...
        flag_assume_valid = (flags & 0b1000000000000000)
        flag_extend = (flags & 0b0100000000000000) != 0
        assert not flag_extend
...
def gitignore_read(repo):
    ...
    if os.path.exists(global_file):
        with open(global_file, "r") as file:
            ret.absolute.append(gitignore_parse(file.readlines()))
...
def cmd_status_branch(repo):
    ...
    if branch:
        ...
        print(f"Head detatched at {object_find(repo, 'HEAD')}")
    ...
    for(abspath, relpath) in clean_paths:
        with open(abspath, "rb") as fd:
            ...
            entry = GitIndexEntry(ctime = (ctime_s, ctime_ns), mtime = (mtime_s, mtime_ns), dev = stat.st_dev, ino = stat.st_ino,
                                  mode_type = 0b1000, mode_perms = 0o644, uid = stat.st_uid, git = stat.st_gid, 
...
def cmd_commit(args):
    ...
    if active_branch:
        with open(repo_file(repo, os.path.join("refs/heads", active_branch)), "w") as file:
            file.write(commit + "\n")
    else:
        with open(repo_file(repo, "HEAD"), "w") as file:
            file.write("\n")